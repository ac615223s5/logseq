...
headlessly with `node .claude/skills/run-logseq/driver.mjs` (a small
...
```bash
node .claude/skills/run-logseq/driver.mjs shot /tmp/logseq-shot.png   # load app, screenshot
node .claude/skills/run-logseq/driver.mjs smoke                       # type blocks, verify persistence
node .claude/skills/run-logseq/driver.mjs eval 'document.title'       # run JS in the page
```
